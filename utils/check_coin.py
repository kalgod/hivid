...
def main():
    ...
    video_list = ["video_30566"] #,"video_31529"
    ...
    for (index, video_name) in enumerate(video_list): #enumerate函数同时遍历索引和目标，这里就是直接遍历video_list的每一项，index从0开始
        ...
        #m=int(input("输入m:"))
        m=8
        plcc,srcc,plcc_idx,srcc_idx,weight,weight_modified =  eval_llm_msort(gtscore,info,frame_path,client,eval_path,eval_path_modified,m)  #除了weight以外都是数，weight是得分数组
