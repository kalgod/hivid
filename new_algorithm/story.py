...
class DataManager:
    ...
    def update_story(self, numbers, story_p, story_t):
        """
        更新或添加故事到对应的数字数组。
        :param numbers: 数字数组，例如 [1, 2, 3]
        :param scores: 故事 "a"
        ...
        """
        key = self.get_key(numbers)
        self.story_map[key] = [story_p,story_t]
...
def send_message_story(image_path, client,info,story_last):
    ...
    The original video informations are {info}
    Your task is as below:
    Based on the frames and informations, analyze the content that this video clip attempts to convey. Then output your answer as "story_partial" and "story_total". (No more than 100 words, and those two should be the same.)
    ...
    The original video informations are {info}
    Your task is as below:
    1. Based on the frames and story and informations, analyze the content that this video clip attempts to convey. Then output your answer as "story_partial". (No more than 100 words.)
    2. Based on the information you just analyzed and the content before this video clip, summarize what story this video has told so far. Then output your answer as "story_total". (No more than 200 words.)
...
def extract_story(res):
    ...
    return story_partial,story_total
...
def mkstory(info,frame_path,client,m):
    ...
    for image_path in groups:
        ...
        for j in range(5): #尝试五次llm请求
            frame=data_man.get_numbers(image_path)
            story_p,story_t=data_man.get_story(frame)
            check=1
            res="Using Cached Results"
            if (story_p==None and story_t==None):
                ...
                print(res)
                story_p,story_t=extract_story(res)
                data_man.update_story(frame,story_p,story_t)
...
def main():
    ...
    for (index, video_name) in enumerate(video_list): #enumerate函数同时遍历索引和目标，这里就是直接遍历video_list的每一项，index从0开始
        ...
        story = mkstory(info,frame_path,client,m)
        print("生成完成。\n",story)
        return 0
